...
@section('title', 'Roadmap - NexusEcom')
@section('header_title', 'Implementação')
...
<div class="space-y-8">
    <div class="text-center py-8">
        ...
        <p class="text-slate-400 font-bold italic mt-2 underline decoration-indigo-500/50 decoration-2 underline-offset-4">Roadmap de Implementação & Novas Funcionalidades</p>
    </div>

    @foreach($phases as $phase)
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl overflow-hidden shadow-xl hover:border-indigo-500/30 transition-all">
            <div class="p-6 border-b border-slate-100 dark:border-slate-700">
                ...
                <div class="mt-4 h-2 bg-slate-100 dark:bg-slate-900 rounded-full overflow-hidden">
                        ...
                        @foreach($phase['technical_details'] as $key => $detail)
                            ...
                            @else
                                <div class="md:col-span-2 mt-2 first:mt-0">
                                    <h4 class="text-xs font-black text-indigo-400 uppercase tracking-widest">{{ $key }}</h4>
                                </div>
                            @endif
                        @endforeach
                    </div>
                        ...
                        @foreach($phase['tasks'] as $taskName => $status)
                            @if(is_string($status))
                                <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest 
                                    {{ $status === 'OK' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20' }}">
                                    {{ $taskName }}: {{ $status }}
                                </span>
                            @else
                                <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-slate-100 dark:bg-slate-900 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                                    {{ $status }}
                                </span>
                            @endif
                        @endforeach
    ...
    <footer class="p-8 bg-slate-800 border border-slate-700 rounded-3xl text-center shadow-inner">
        <p class="text-slate-400 text-sm font-medium italic">
            "NexusEcom ⚡: Performance extrema com Blade + Alpine.js."
        </p>
    </footer>
</div>
@endsection
